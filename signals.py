...
class Fx:
    ...
    def _evaluate_macd(self):
        ...
        if not col_xa or not col_xb:
            return 'Stay', 'NA'
        buy_signal = self.candles.iloc[-1][col_xa.get('name')]
        sell_signal = self.candles.iloc[-1][col_xb.get('name')]
    ...
    def _evaluate_stoch(self):
        ...
        if not col_stk:
            return 'Stay', 'NA'
        self.candles = _add_signal(self.candles, col_stk.get('name'), xa=80, xb=20)
        # actual evaluate
        cols = self.candles.columns.to_list()
